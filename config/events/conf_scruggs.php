<?php
	return [

		"metas" => [
			"title" => "Scruggs 70th Birthday Cruise",
			"description" => "Join us for Mr. Scruggs 70th Birthday Cruise!",
			"keywords" => "Scruggs 70th Birthday Cruise, culture, parties, 2026",
			"canonical" => "https://goyollo.com/scruggs-70th",
		],

		"event" => [
			"title" =>"Steve Scruggs 70th Birthday Cruise",
			"subtitle" =>"3-Day Cruise to The Bahamas from Miami",
			"location" =>"Bahamas",
			"date" =>"August 1-7, 2026",
			"date_start" =>"August 1, 2026",
			"img_url" =>"../../images/slides/splash-ghana-cta.jpg",
			"img_alt" =>"Ghana Takeover 2026",
		],

		"geolocation" =>[

			"hotel1"=> array(
				"title" 			=>"4 Star Hotel TBA",
				"soldout"           =>false,
				"img_url"			=>"//dynamic-media-cdn.tripadvisor.com/media/photo-o/1c/de/d6/14/exterior.jpg?w=500&h=-1&s=1",
				"img_default"		=>true, //" ", true
				"address"			=>"",
				"location"			=>"Ghana, Africa",
			),

		],

		"people" =>[

			"diamond"=> array(

				"Group of 4" 			=>"$1325",
				"Group of 3"			=>"$2799",
				"Group of 2"			=>"$2799",
				"Individual"			=>"$3,300"
			),

			"emerald"=> array(
				"Group of 4" 			=>"$1075",
				"Group of 3"			=>"$1205",
				"Group of 2"			=>"$1310",
				"Individual"			=>"$1945"
			),

		],

		"package_items" =>[
			"diamond_items" => array(
				"7 days / 6 nights in Ghana" => "checked",
				"Explore Accra, Cape Coast & Elmina" => "checked",
				"Daily Breakfast" => "checked",
				"2 Dinners & 2 Lunches Included"	=> "checked",
				"Air-Conditioned Charters"	=> "checked",
				"All Tours"	=> "checked",
				"Airport Transfers" => "checked",
				"YOLLO Swag Bag" => "checked"
			),



			"emerald_items" => array(
			/*
				"4 Days / 3 Nights Hotel Accommodations"    => "checked",
				"The Best of the Best Fashion Shows"	    => "checked",
				"11th Annual Winter White Party"		    => "checked",
				"11th Annual \"LOL\" Comedy Show"		    => "checked",
				"The Rep Your City Party"	    => "checked",
				"Freaky Friday PJ Party"				    => "checked",
				"Access To The Exclusive Super Lounges Each Night"	    => "checked",
				"Access To The Love Jones Poets Corner Lounge"			=> "checked",
				"The \"Mid-Night Mountain\" Indoor Pool Party"			=> "checked",
				"Access To The Caribbean Lounge"				        => "checked",
				"Access To The 80's & 90's Old School Lounge"				=> "checked",
				"Tickets To All Additionally Added Events"					=> "checked",
				"YOLLO Swag Bag"							    => "checked"
			*/
			)
		], 


	]
?>